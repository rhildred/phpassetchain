...
EOF;
    ...
    $oStmtStems = $file_db->prepare("INSERT INTO stemsIndex (sStem, sFile, nOffset) VALUES(?, ?, ?)");
  ...
  static function getMatches($sQuery){
    ...
    $file_db->setAttribute(PDO::ATTR_ERRMODE,
                        PDO::ERRMODE_EXCEPTION);
    $oStmt = $file_db->prepare("SELECT sWord FROM wordsIndex WHERE sWord LIKE '%' || ? || '%' ORDER BY nFrequency");
